...
test("vinculos mesma propriedade e mudanca do lote derivam local de todos os animais", async () => {
  ...
  const lot = (await f.lot.create("a", "p", { tagSuffix: "A", name: "L", paddockId: p1.id })).lot;
  const animal = (await f.animal.create("a", "p", { name: "101", lotId: lot.id })).animal;
...
for (const [account, property] of [["a", "q"], ["b", "r"]]) test(`bloqueia pais cross-scope ${account}/${property} na criacao e edicao`, async () => {
  ...
  const l = (await f.lot.create(account, property, { tagSuffix: "A", name: "L" })).lot;
  assert.equal((await f.lot.create("a", "p", { tagSuffix: "A", name: "L", paddockId: p.id })).status, "invalid");
  assert.equal((await f.animal.create("a", "p", { name: "1", lotId: l.id })).status, "invalid");
  const localLot = (await f.lot.create("a", "p", { tagSuffix: "A", name: "L" })).lot;
  const animal = (await f.animal.create("a", "p", { name: "1" })).animal;
...
test("arquivamento bloqueia filhos ativos; mover ou desvincular permite", async () => {
  ...
  const l = (await f.lot.create("a", "p", { tagSuffix: "A", name: "L", paddockId: p.id })).lot;
  const a = (await f.animal.create("a", "p", { name: "1", lotId: l.id })).animal;
...
test("nao aceita vinculo inexistente ou arquivado em entidade ativa", async () => {
  const f = fixture();
  const l = (await f.lot.create("a", "p", { tagSuffix: "A", name: "L" })).lot;
  await f.lot.archive("a", "p", l.id);
  assert.equal((await f.animal.create("a", "p", { name: "1", lotId: l.id })).status, "invalid");
  assert.equal((await f.animal.create("a", "p", { name: "1", lotId: "missing" })).status, "invalid");
  assert.equal((await f.lot.create("a", "p", { tagSuffix: "A", name: "L", paddockId: "missing" })).status, "invalid");
});
