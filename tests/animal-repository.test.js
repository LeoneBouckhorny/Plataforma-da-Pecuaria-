...
test("animal CRUD, listagem e ciclo de status preservam ID", async () => {
  ...
  const result = await repo.create("a", "p", { tag: "101", name: "Igual" });
...
test("animal isola conta e propriedade mesmo com nomes/brincos iguais", async () => {
  const f = fixture(); const repo = f.animal;
  const p = (await repo.create("a", "p", { tag: "101", name: "Igual" })).animal;
  const q = (await repo.create("a", "q", { tag: "101", name: "Igual" })).animal;
  const r = (await repo.create("b", "r", { tag: "101", name: "Igual" })).animal;
...
test("animal bloqueia transferencia, conta forjada, propriedade ausente e colisao ID", async () => {
  const f = fixture(); const repo = f.animal;
  const entity = (await repo.create("a", "p", { tag: "101", name: "Igual" })).animal;
  ...
  assert.equal((await repo.create("b", "p", { tag: "101", name: "Igual" })).status, "invalid");
  assert.equal((await repo.create("a", "missing", { tag: "101", name: "Igual" })).status, "invalid");
  repo.options.idFactory = () => entity.id;
  assert.equal((await repo.create("a", "p", { tag: "101", name: "Igual" })).status, "failed");
