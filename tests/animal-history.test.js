...
test("vinculo explicito captura snapshots e timeline combina eventos, nascimento e pesagens", async () => {
  const f = fixture(); const a = (await identified(f, "a", "p", { tag: "101", name: "Original", birthDate: "2025-08-10" })).animal;
  ...
  assert.equal((await f.animal.update("a", "p", a.id, { tagNumber: "184", name: "Matriz 184" })).status, "saved");
  ...
  assert.ok(beforeItems.every((item) => item.animalTagSnapshot === "0101A" && item.animalNameSnapshot === "Original"));
...
test("nao infere animalId por tag no draft, finalizacao, legado ou historico", async () => {
  const f = fixture(); const a = (await identified(f, "a", "p", { tag: "101" })).animal;
  const draft = Draft.normalizeDraftData({ animals: [{ id: "item", tag: a.tag, weight: "450" }] });
  ...
  await repo.saveCompletedSession(snapshot(null, {}, [{ id: "row", tag: a.tag, weight: "450" }]));
...
for (const scenario of ["missing", "other-property", "other-account", "archived", "wrong-lot"]) test(`finalizacao revalida ${scenario}`, async () => {
  const f = fixture();
  const l = (await f.lot.create("a", "p", { tagSuffix: "B", name: "L" })).lot;
  const a = (await identified(f, scenario === "other-account" ? "b" : "a", scenario === "other-account" ? "r" : scenario === "other-property" ? "q" : "p", { tag: "101" })).animal;
